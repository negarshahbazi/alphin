...
function PureBuild() {
  ...
  const CuirNoirDinamica = useSelector((state) => state.pures.CuirNoirDinamica);
  const CuirNoirPerfore = useSelector((state) => state.pures.CuirNoirPerfore);

  const images = [img1, img2, img3, img4, img6, img7];
  const imagesName = ['Conduite', 'Confort', 'Design', 'Media', 'Perso', 'Securite'];

  const equipmentsData = useSelector((state) => state.pures.equipmentsData);
  const [totalEquipmentPrice, setTotalEquipmentPrice] = useState(0);

  const accessoriesData = useSelector((state) => state.pures.accessoriesData);
  ...
  const changeSellerie = (color) => {
    let selectedSellerie = CuirNoirPerfore;
    switch (color) {
      case 'Dinamica':
              ...
              <div className='row'>
                ...
                <div className="col-md-6  container d-flex justify-content-center align-items-center ">
                  <div className='row d-flex justify-content-center align-items-center text-center  mb-5' >
                    ...
                    <div className='col-3 ' >
                      <img className='p-1 shadowCard rounded shake1' src={cuir2} style={{ width: '90%', height: '100px', border: '5px solid black' }} onClick={() => { changeSellerie('Dinamica'); changePrixSellerie('Dinamica'); }} />
                      <span>Sièges baquets en cuir noir et Dinamica</span><br />
                      <b>{Dinamica} €</b>
                    </div>
                    <div className='col-3'>
                      <img className='p-1 shadowCard rounded shake1' src={cuir3} style={{ width: '90%', height: '100px', border: '5px solid black' }} onClick={() => { changeSellerie('Perfore'); changePrixSellerie('Perfore'); }} />
